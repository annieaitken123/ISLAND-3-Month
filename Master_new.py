...
for movie in movies:
    ...
    ns.send_event( 'evt_', label="start", timestamp=egi.ms_localtime(), table = {} )
    t0 = libtime.get_time()
...
#  Wait until the participant presses any key to start.
    while movie.status != visual.FINISHED:
        ...
        if key in ['Escape', 'escape']:
            ...
            ns.send_event( 'evt_', label="esc", timestamp=egi.ms_localtime(), table = {} )
            break
    ns.send_event( 'evt_', label="end", timestamp=egi.ms_localtime(), table = {} )
